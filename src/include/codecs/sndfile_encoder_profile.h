...
#ifndef CODECS_SNDFILE_ENCODER_PROFILE_H
#define CODECS_SNDFILE_ENCODER_PROFILE_H
...
namespace codecs
{
class SndFileEncoderProfile : public EncoderProfile
{
    public:
        /*!
        Valid output formats are extracted from sndfile.c function "sf_format_check" Format names are from command.c
        */
        enum class OutputFormat : SNDFILE_ENCODER_OUTPUT_FORMAT_DATA_TYPE
        {
            /*! WAV (Microsoft) */
    ...
    private:
        ...
        static const std::unordered_map<const OutputFormat, std::string> output_format_to_profile_extension;
    ...
    public:
        ...
        SndFileEncoderProfile(void);

        void LoadDefaultProfile(Profile default_profile);

        void SetOutputFormat(SNDFILE_ENCODER_OUTPUT_FORMAT_DATA_TYPE output_format);
